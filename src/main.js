...
//
//  Changelog v3 → v4:
//   + Pause menu (ESC) — Resume / New Game / Hangar
//   + Back button на всех overlay-экранах
//   + Плавный поворот корабля к ближайшему врагу (s.aimAng → s.ang)
//   + Стрельба только когда корабль смотрит на врага (±20°)
//   + Система сканирования: 50 энергии, -10/скан (Space/кнопка)
//   + 3 типа астероидов: poor(2)/medium(5)/rich(10) ore
//   + Механика добычи: силовой луч, ~3 сек, не прерывается в зоне
//   + Мини-карта: показывает revealed объекты с цветом по типу
//   + Ore → Credits конвертация в конце рана
//   + META.ore — накопительный ресурс
// ═══════════════════════════════════════════════════════
...
const WSX = WW/2, WSY = WH/2;
const SPAWN_RADIUS  = 380;
const SCAN_RADIUS   = 420;   // радиус скана в мировых единицах
const SCAN_MAX      = 50;    // максимальная энергия
const SCAN_COST     = 10;    // стоимость одного скана
const SCAN_REGEN    = 3;     // единиц в секунду
const MINE_RANGE    = 75;    // дистанция начала добычи
const MINE_RANGE_SQ = MINE_RANGE * MINE_RANGE;
const MINE_CONT     = 130;   // дистанция продолжения (не прерывается)
const MINE_TIME     = 3000;  // мс на добычу
const ORE_VALUE     = { poor:2, medium:5, rich:10 };
const ORE_PER_CREDIT = 2;
...
// ═══════════════════════════════════════════════════════
//  WEB3  —  VoidOreMinter @ Base Sepolia
//  0.0001 ETH = 1000 ore  (pricePerOre = 100 gwei)
// ═══════════════════════════════════════════════════════
...
const W3 = { provider:null, signer:null, contract:null, address:null, connected:false, chainOk:false };
let _toast='', _toastCol='#44aaff', _toastLife=0;

function w3Toast(msg,col='#44aaff'){ _toast=msg; _toastCol=col; _toastLife=3200; }

async function w3Init(){
...
async function w3Connect(silent=false){
...
async function w3BuyOre(oreAmount){
...
function w3Credit(n){
  META.ore+=n; META.credits+=n; saveMeta(); updHUD();
  if(G&&G.ph==='play'){ptcl(G.s.x,G.s.y,'#44ffaa',30,5,4);ring(G.s.x,G.s.y,'#44ffaa',60);}
  w3Toast('+'+n.toLocaleString()+' ore!','#44ffaa');
}

function w3Short(a){ return a?a.slice(0,6)+'…'+a.slice(-4):''; }

function w3Migrate(){
  if(!W3.address) return;
  const key='vs_meta_'+W3.address.toLowerCase();
  try{const ex=localStorage.getItem(key);if(ex){META=JSON.parse(ex);if(!META.owned.includes('vanguard'))META.owned.push('vanguard');if(!META.ore)META.ore=0;updHUD();return;}}catch{}
  localStorage.setItem(key,JSON.stringify(META));
}

// ─── W3 рендер ─────────────────────────────────────────

function w3DrawToast(dt){
...
function w3DrawShopPanel(){
  // Панель внизу Hangar — Buy Ore
...
const STAR_DATA = STAR_LAYERS.map(layer =>
...
const NEBS = Array.from({ length:8 }, () => ({
...
function saveMeta() {
  try {
    ...
    if(W3.address) localStorage.setItem('vs_meta_'+W3.address.toLowerCase(),d);
  } catch(e) {}
}
let META = loadMeta();
  ...
  function tryPlace(type, attempts=40) {
    for (let a=0;a<attempts;a++) {
      ...
      if (type==='asteroid') {
        ...
        o.driftSpd = rn(0.18);
        o.shade    = ri(3);
        o.revealed = false; // виден на мини-карте только после скана
      } else if (type==='nebula') {
        o.hue      = 180+ri(120);
        o.driftAng = rn(Math.PI*2);
        o.driftSpd = rn(0.06);
      ...
      obs.push(o); return true;
    }
    return false;
  }

  const am = G.location ? G.location.asteroids : 1;
  const aCount = Math.round((28+wave*2)*am), nCount = 6+wave, cCount = Math.round((18+wave*3)*am);
...
function mkG() {
  ...
  return {
    ...
    // ── v4 additions ────────────────────────────────────
    ore:         0,           // руда добытая за ран
    shopFrom:    'menu',      // откуда открыт магазин
    scanEnergy:  SCAN_MAX,    // текущая энергия сканирования
    scanRegen:   0,           // аккумулятор регенерации
    scanPulse:   0,           // анимация кольца скана (1→0)
    scanPulseR:  0,           // текущий радиус кольца
    revealedEns: [],          // [{x,y,tp,life}] — радарные метки врагов

    mining: null,             // {obs, t, maxT} — активная добыча

    // ── Extraction ──────────────────────────────────────
    extraction: {
      zone: null,       // { x, y, radius }
    ...
    s: {
      ...
      invT:0,
      ang:    -Math.PI/2,   // текущий угол (визуальный, плавный)
      aimAng: -Math.PI/2,   // целевой угол (к ближайшему врагу)
...
// ── Мир и спавн ───────────────────────────────────────
function genWorld() {
  G.obs = genObstacles(1);
  G.s.x=WSX; G.s.y=WSY; G.s.vx=0; G.s.vy=0;
  G.cam.x=WSX-W/2; G.cam.y=WSY-H/2;
}

// Spawn rates: шанс спавна при проверке (каждые ~2 сек)
const SPAWN_RATES = { seek:0.6, fast:0.3, heavy:0.15 };
const BOSS_CFG = {
...
const MAX_BOSSES_PER_RUN  = 2;
const BOSS_SPAWN_CHANCE   = 0.04;

function trySpawnEnemies() {
  // Считаем видимых врагов (viewport)
  const visible = G.ens.filter(e=>isVis(e.x,e.y,e.r));
  if (visible.length >= MAX_VISIBLE_ENEMIES) return;

  const em = G.location ? G.location.enemies : 1;
  // Бросаем кости для каждого типа
  for (const tp of ['seek','fast','heavy']) {
    if (Math.random() < SPAWN_RATES[tp] * em) {
      spawnEn(tp); return; // один враг за тик
    }
  }
}

function trySpawnBoss() {
  if (G.boss) return;              // уже есть босс на карте
...
function killBoss() {
  const b=G.boss;
  if (!b) return;
  ptcl(b.x,b.y,'#ffdd44',25,6,4); ring(b.x,b.y,'#ffdd44',b.r*3);
  G.score+=b.pts;
  // Дроп руды
  ...
  // XP gems
  const gc=4;
  for (let j=0;j<gc;j++) gem(b.x+rn(24)-12,b.y+rn(24)-12,Math.ceil(b.xp/gc));
  updHUD();
  G.boss=null;
}

// ── Extraction ────────────────────────────────────────
function initExtraction(waveDuration) {
  const margin = 120;
...
function killEn(i) {
  ...
  ptcl(e.x,e.y,col,13,4,3.5); ring(e.x,e.y,col,e.r*2.6);
  const gc=1+Math.floor(e.xp/10);
  for (let j=0;j<gc;j++) gem(e.x+rn(16)-8,e.y+rn(16)-8,Math.ceil(e.xp/gc));
  G.score+=e.pts; updHUD(); G.ens.splice(i,1);
}
...
function update(dt) {
  ...
  if (s.u.chain>0 && s.cT<=0 && facingTarget && visEns.length>0) {
    ...
    targets.forEach(e=>{
      ...
      if (e.hp<=0){const idx=G.ens.indexOf(e);if(idx>=0)killEn(idx);}
    });
...
function draw(ts) {
  ...
  // Toast поверх всего
  w3DrawToast(Math.min(ts-ts0,60));
  C.restore();
}
...
function startGame() {
  ...
  genWorld();initExtraction(G.location.dur);
